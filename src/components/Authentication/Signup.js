...
const Signup = () => {
  ...
  const submitHandler = async () => {
    ...
    console.log(name, email, password, dp);
    try {
      const config = {
        headers: {
  ...
  const postDetails = (dp) => {
    ...
    if(dp===undefined){
     ...
     if (dp.type === "image/jpeg" || dp.type === "image/png") {
      ...
      data.append("cloud_name","dpiowp70c");
      fetch("https://api.cloudinary.com/v1_1/dpiowp70c/image/upload", {
        method: "post",
        body: data,
      })
        .then((res) => res.json())
